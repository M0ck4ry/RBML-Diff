...
def get_loader(cfg):
    ...
    test_dataset = instantiate_from_config(cfg.test_dataset.Kvasir)
    test_dataset_expand = SampleDataset(full_dataset = instantiate_from_config(cfg.test_dataset.CVC_300), interval=10)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
    test_dataset_expand = SampleDataset(full_dataset = instantiate_from_config(cfg.test_dataset.CVC_ClinicDB), interval=10)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
    test_dataset_expand = SampleDataset(full_dataset = instantiate_from_config(cfg.test_dataset.CVC_ColonDB), interval=10)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
    test_dataset_expand = SampleDataset(full_dataset = instantiate_from_config(cfg.test_dataset.ETIS_LaribPolypDB), interval=10)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
